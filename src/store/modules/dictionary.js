...
const dictionary = {
  ...
  mutations: {
    ...
    // },
    // 以serviceType为索引
    setServiceType(state, {serviceType, data}) {
      state[serviceType] = data.map(item => ({ text: item.pname, value: item.pcode }))
    }
  },
  actions: {
    async fetchServiceType({ commit, state }, serviceType) {
      if (state[serviceType]) return state[serviceType]
      let data
      if(AppBridge.isApp) {
        data = await new Promise(resolve => {
          AppBridge.getInitData({ type: serviceType }, res => {
            resolve(res.data[serviceType])
          })
        })
      }
      else {
        data = await api.dictionary({ servicetype: serviceType }).then(res => res.data)
      }
      commit('setServiceType', {serviceType, data})
