<template lang="html">
    <div class="view">
        ...
        <div class="c-layer">
            <c-header ref="transparentHeader" class="header transparent" :style="`background: rgba(65, 70, 81, ${headerRgba}); opacity: ${headerOpacity}`" :title="appName" :show="true">
        ...
        <scroller v-if="homeData.apiFinish" class="c-view-content" id="scrollerContent" :enableRefresh="true" :on-refresh="refresh" :offset="100" :bounce="60" :maxScrollerHeight="160" :power="2" @refreshFinsh="refreshFinsh" @onScroll="onScroll" @onPulling="onPulling">
            ...
            <div class="section tobe-repay" v-if="homeData.repayProducts.length > 0">
                <mt-cell title="待还账单" is-link @click.native="$root.openUrl(repayUrl)">{{homeData.repayProducts.length}} 笔</mt-cell>
            </div>
            <!-- 专属推荐（new卡） -->
            ...
            <!-- 在线客服 -->
            <div class="section kefu-content" v-if="homeData.apiFinish && homeData.withdrawProducts.length <= 0 && onlineServiceUrl" @click="$root.openUrl({url: onlineServiceUrl, title: '在线客服'})">
