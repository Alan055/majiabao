...
export default {
  ...
  methods: {
    ...
    handleInfoClick2(item, i) {
      ...
      if (item.openlink && item.openlink.includes("XM=0")) { // 点击的是微信链接 跳到微信去
        this.$root.openUrl({
          type: "open",
          url: item.openlink,
          isFull: false,
      ...
      if (i == 0) { // 点击第一条  行业资讯
        this.sinaAds.click({currEvent: this.adsInfo.news.index.clickArea});
        url = item.openlink;
      } else if (i == 1) { // 点击第二条  防骗
        this.sinaAds.click({currEvent: this.adsInfo.news.index.clickArea2});
        url = item.openlink;
      } else { // 点击其他
        url = item.openlink + "?id=" + item.id;
      }
      console.log("item2", item);
      this.$root.openUrl(url, {title: "详情"});
    }
  },
  created() {
