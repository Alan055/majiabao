  ...
  <div class="view-step2 view-public-info-step c-bg-gray">
    ...
    <div class="c-view-content">
      <div class="content-wrap" v-min-height>
        ...
        <div class="c-section">
          <!--婚姻状态-->
          <form-control type="select" title="婚姻" v-model="formData.maritalstatus" source="20011003" @input="selectedCallBack" name="maritalstatus" v-validate="'required'" v-sina-ads="stat.publicInfo.contacts.maritalstatus.marriage" />
          <!--已婚-->
          <div class="c-box" v-if="formData.maritalstatus == 'D02'" @click='handleClickTag1'>
            <choose-contacts title="配偶" v-model="formData.family" name="family" v-validate="'required'" />
          </div>
          <div class="c-box" v-else>
            <div class="relationShipClick-box" @click='handleClickTag3'>
              <choose-contacts title="请选择紧急联系人" v-model="formData.family" name="family" v-validate="'required'" />
            </div>
            <div class="relationShip-box" @click='handleClickTag2'>
              <form-control type="select" title="与联系关系" @input="selectedCallBack2" v-model="formData.extratype" source="20011041" name="extratype" v-validate="'required'" />
            </div>
          </div>
          <div class="error-tip" v-if="errorInfo.family && fields.family && !fields.family.dirty">{{errorInfo.family}}</div>
        </div>
        <div class="c-section">
          <!-- <choose-contacts title="家人" v-model="formData.family" name="family" v-validate="'required'" v-sina-ads="stat.publicInfo.contacts.family" /> -->
          <!-- <div class="error-tip" v-if="errorInfo.family && fields.family && !fields.family.dirty">{{errorInfo.family}}</div> -->
          <!-- <div class="error-tip" v-if="fields.family && !fields.family.dirty">test</div> -->
          <choose-contacts title="同事" v-model="formData.friend" name="friend" v-validate="'required'" v-sina-ads="stat.publicInfo.contacts.friend" />
          <div class="error-tip" v-if="errorInfo.friend && fields.friend && !fields.friend.dirty">{{errorInfo.friend}}</div>
          <choose-contacts title="朋友" v-model="formData.colleague" name="colleague" v-validate="'required'" v-sina-ads="stat.publicInfo.contacts.colleague" />
          <div class="error-tip" v-if="errorInfo.colleague && fields.colleague && !fields.colleague.dirty">{{errorInfo.colleague}}</div>
        </div>

        <div class="c-button-group">
          <mt-button type="primary" size="large" class="btn-lg btn-primary btn-round" :disabled="!finished" @click.native="defendClick.wrap(submit)" v-sina-ads="stat.publicInfo.contacts.submit">{{sendBack ? '提交' : '下一步'}}</mt-button>
        </div>
      </div>
    </div>
      ...
      AppBridge.getUserInfo({}, res => {
        if (res.code == '200' && res.data && res.data.mobile) {
          api.publicInfo.newbiePackets({
            ...
            productid: `2001`,
            type: 2, //1身份证信息、2人际关系、3基础信息、4完成提交申请
