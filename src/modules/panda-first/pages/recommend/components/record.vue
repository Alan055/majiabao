  ...
  <div>
    <div class="record-wrap">
      <div class="no-record" v-if="isShowNodata">
        ...
        <div class="title">当前暂无可借款机构，请保持关注。更多金融机构正在接入之中··</div>
        <div v-show="thirdproducts" class="sub-title">建议您尝试线下贷款，最高额度可达20万元。</div>
        <div v-show="thirdproducts" class="triangle-border-down">
          <span></span>
        </div>
      </div>
      <div class="has-record" v-else>
        ...
        <div class="cricle-half">
          ...
          <p class="half-sub-title">{{countMoney.toFixed(2)}}</p>
        </div>

        <redBagBox :res="resdata.welfareInfo" style="padding: 0 12px;background: #fff;  box-shadow: 0 2px 16px 0 rgba(0, 0, 0, 0.06);"></redBagBox>
        <div class="list-wrap">
  ...
  export default {
    ...
    computed: {
      data() {
        ...
        let count = 0;
        this.resdata.withdrawProducts.forEach(item => {
   ...
   .handle-button {
    ...
    overflow: hidden;
     ...
     /deep/ &.blue {
      ...
      box-shadow: 0 1px 6px 0 rgba(88, 130, 255, 0.4);
      border-radius: 16px;
    }
  }
</style>
