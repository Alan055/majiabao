<template lang="html">
    <mt-cell class="cp-choose-contacts" :title="title" :is-link="!readonly" @click.native="chooseContacts">
        ...
        <div v-else class="c-table-row c-light value-grid">
            ...
            <dd class="c-tl name-cell" v-if="readonly || !value">{{value.contactName}}</dd>
            <dd class="c-tl name-cell" v-else @click.stop><input @blur="updateValue($event.target.value)"
                                                                 :value="value.contactName" maxlength="15"/></dd>
            <!--<dt class="c-light">{{computedValue.contactPhone}}


            </dt>-->
            <dt class="c-light">{{value.contactPhone}}</dt>
...
<script>
    ...
    export default {
        props: ["value", "title", "readonly", "before", 'name', 'onError'],
        data() {
            return {
                selected: null
            };
        ...
        methods: {
            async chooseContacts() {
                ...
                if (this.before) this.before(this.name)

                await checkContactPermission()

                appBridge.chooseContacts(res => {
                    // res = JSON.parse(res);
                    if (res.code == 200) {
                        ...
                        if (!new RegExp("[\\u4E00-\\u9FFF]+", "g").test(name)) {
                            if (this.onError) this.onError(this.name, `不可选择名字为纯数字或纯英文的联系人`)
                            this.$emit("input", "");
                            return;
                        }
                        ...
                        if (!/^1\d{10}/.test(phone)) {
                            ...
                            if (this.onError) this.onError(this.name, `需选择含有正确手机号码的联系人，请重选`)
                            this.$emit("input", "");
                            return;
                        }
                        if (phone == this.userInfo.mobile) {

                            if (this.onError) this.onError(this.name, `联系人号码不能与注册号码相同，请重选`)
                            this.$emit("input", "");
                            return;
                        }
                        if (/^(145|147)/.test(phone)) {

                            if (this.onError) this.onError(this.name, `不支持以147、145开头的联系人手机号，请重选`)
                            this.$emit("input", "");
                            return;
                        }
