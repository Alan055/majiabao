    ...
    <div class="view-step3 view-public-info-step c-bg-gray" ref="step3Content">
        ...
        <div class="c-view-content">
            <div class="content-wrap" v-min-height>
                ...
                <div class="c-section">
                    <!-- <form-control type="select" title="所属行业" v-model="formData.occupations" source="20011005" /> -->
                    ...
                    <div v-if="tendencyList[0] && tendencyList[0].answerDTO" class="c-grid-item">
                        <dl>
                            <dt class="title">借款偏好</dt>
                            <dd @click="showTendency = !showTendency">
                                <span :class="['c-right', formData.tendencyRightText == '已选择'?'ready':'']">{{formData.tendencyRightText || '选择'}}</span>
                                <c-icon class="icon-16 step-arrow-right" type="arrowRight"/>
                            </dd>
                        </dl>
                    </div>

                    <choose-tendency v-if="tendencyList[0] && tendencyList[0].answerDTO"
                                     slot="bankcard"
                                     :show="showTendency"
                                     :list="tendencyList"
                                     :isTwo="isTendencyTwo"
                                     :active="formData.tendencyCode"
                                     @changeTwo="isTendencyTwo = false"
                                     @chooseFinshed="chooseFinshed"
                                     @toggleDone="chooseDone"
                    ></choose-tendency>
                </div>
                ...
                <div class="c-section">
                    ...
                    <!-- <form-control type="select" title="婚姻状况" v-model="formData.maritalstatus" source="20011003" name="maritalstatus" v-validate="'required'" v-sina-ads="stat.publicInfo.userBasicInfo.marriage" /> -->
              ...
              <!--  <textarea v-model="formData.companyaddr" v-no-emoji v-max-char-len="50"
                          v-sina-ads="stat.publicInfo.userBasicInfo.companyAddress"
                          @click="textClick"
                          placeholder="测试信息.........." maxlength="50" name="companyAddress"
                          v-validate="'required|minChar:10'" @focus="textOnFource"
                          @blur="companyAddressFocused=false"></textarea>

                <p style="height: 50px; background: red;"    @click="textClick" >测试信息</p>-->
        ...
        methods: {
            ...
            textOnFource(ev) {
                ...
                window.setTimeout(function () {
                    ...
                    // this.$refs.step3Content.scrollTop = 300+'PX'
                ...
                helper.get(`${userid}-userBasicInfo`).then(value => {
                    ...
                    if (value && this.checkData(value)) {
                        ...
                        this.fetchData().finally(() => {
                            ...
                            // console.log("本地数据值：" + JSON.stringify(value, null, 2));
                            // console.log("服务器数据值：" + JSON.stringify(this.formData, null, 2));
