  ...
  <div class="view-step1 view-public-info-step c-bg-gray" ref="view">
    ...
    <div class="c-view-content" ref="content">
      <div class="content-wrap" v-min-height>
        <step-progress step=1 v-if="!sendBack"/>
        <div class="c-section id-card-info" v-if="formData.posimageurl" @click="watchResize">

          <form-control title="<span class=between>姓</span><span class=between>名：</span>"  v-model="formData.cardname" maxlength="15" :disabled="identityIsReadonly"
                        :readonly="identityIsReadonly" v-sina-ads="stat.publicInfo.idCard.idName"/>
          <form-control title="<span class=justify>身份证号码</span>：" v-model="formData.cardno" maxlength="18" :disabled="identityIsReadonly"
        ...
        <div class="id-card-input">
          <div class="card-list" v-sina-ads="stat.publicInfo.idCard.pos">
            ...
            <photo-scan v-model="formData.posimageurl" :readonly="!cardImageEditable" :ocr="cardNumberEditable"
                        :showblur='showBlur2' type="front" v-on:idinfoChange="resolveFileId" ref="idCardFront"
                        name="posimageurl" v-validate="'required'">
              <div class="default-img" slot="default-img" :style="`height: 105px; background: url(${require('../images/renxiang.png')}) no-repeat;background-size: 100% 100%;`">
                <img src="../images/paishe.png" alt="">
          ...
          <div class="card-list" v-sina-ads="stat.publicInfo.idCard.opp">
            ...
            <photo-scan v-model="formData.oppimageurl" :readonly="!cardImageEditable" type="back" :showblur='showBlur'
                        v-on:idinfoChange="resolveFileId" ref="idCardBack" name="oppimageurl" v-validate="'required'">
              <div class="default-img" slot="default-img" :style="`height: 105px; background: url(${require('../images/guohui.png')}) no-repeat;background-size: 100% 100%;`">
                <img src="../images/paishe.png" alt="">
                <p class="paishe">点击扫描</p>
              </div>
            </photo-scan>
        ...
        <p class="has-question" v-if="onlineServiceURL" @click="$root.openUrl({url: onlineServiceURL, title: '在线客服'})">遇到问题？</p>

        <div class="c-button-group">
          <mt-button type="primary" size="large" class="btn-lg btn-primary btn-round" @click.native="submit()"
                     v-sina-ads="stat.publicInfo.idCard.submit" :disabled="!finished">评估额度
          </mt-button>
        </div>
      </div>
...
<script>
  ...
  import photoScan from "./components/photoScan-test";
      ...
      configSetting() {
        ...
        AppBridge.getInitData({}, res => {
          ...
          if (res.code == '200' && res.data && res.data.config && typeof res.data.config.face_recognition_method != 'undefined') {
            ...
            console.log('活体类型 人脸识别方式==》' + res.data.config.face_recognition_method);
