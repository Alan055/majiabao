...
async function checkLocationPermission(realcheck) {
    ...
    if (!permission) {
        ...
        await MessageBox({
                title: '温馨提示',
                message: window.sinaif_global && window.sinaif_global.appName + '想获取您的位置信息，请允许，否则会影响您的借款相关操作',
                confirmButtonText: '我知道了',
                closeOnClickModal: false
            })
            .then(action => {
    ...
    return await new Promise(resolve => {
        AppBridge.permission({
            ...
            console.log('res------------>>>>>>>>' + res)
            console.log('res.data.grant------------>>>>>>>>' + res.data.grant == 'granted')
            resolve(res.data.grant == 'granted')
        })
    });
