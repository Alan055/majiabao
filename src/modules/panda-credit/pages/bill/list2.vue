...
export default {
  ...
  methods: {
    gotoAgreements() {
      ...
      if (this.$root.isApp) {
        let path = location.href.split("/panda-credit")[0];

        //  pages/panda-credit#/bill
        // let url = path + `/pages/my/#/agreements/loanList/${this.routeParams.productid}/${this.routeParams.orderid}`;

        let url =
          path +
          `/panda-credit/#/agreements/loanList/${this.routeParams.productid}/${
            this.routeParams.orderid
          }`;
    ...
    this.$root.getRouteData().then(params => {
      ...
      Promise.all([this.getBillList1(), this.getBillList2()]).then(values => {
        ...
        billList.forEach(item => {
          if (item.dueamount) {
            count = new Number(
              parseFloat(count) + parseFloat(item.dueamount)
            ).toFixed(2);
