<template>
    <div>
        <div class="refuseList-warp c-bg-gray">
            ...
            <li class=" c-border-b item" v-sina-ads='{
                    currEvent:stat.recommend.refuse.click,
                    currentEventParams:{
                        name:item.name,
                    }
                    }' v-for="(item,index) in refuseList" :key="index" @click="handleClick(item.nextpage)">
                ...
                <p class="text" v-html="item.text2[0].content"></p>
                </div>
                <div class="item-info">
                <div class="info">
                    <h3>{{item.maxamount | money2Fixed}}</h3>
                    <p>{{item.text1[0].content}}</p>
                </div>
                <div class="link">
                    <span>{{item.text3[0].content}}</span>
                </div>
                </div>
            </li>
            </div>
        ...
        <div class="c-view-content refuse-noList c-absolute c-bg-white" v-if="noRefuseData">
            <div class="refuse-noList-img c-aspectratio w-180-140">
                <div class="c-aspectratio-content2">
                    <img style="width: 100px;" src="../images/bill-nodata.png" />
                </div>
            </div>
            <div class="refuse-noList-text c-tc c-font-l">
                您暂时没有拒件记录
            </div>
        </div>
    </div>
