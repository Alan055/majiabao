    ...
    <div class="c-view-content">
      <div class="content-wrap" v-min-height>
        ...
        <div class="c-section">
          ...
          <!--已婚-->
          <div class="c-box c-border-b" v-if="formData.maritalstatus == 'D02'" @click='handleClickTag1'>
          ...
          <div class="c-box" v-else>
            <div class="relationShip-box c-border-b" @click='handleClickTag2'>
              <form-control type="select" title="直系亲属" @input="selectedCallBack2" v-model="formData.extratype" source="20011041" name="extratype" v-validate="'required'" />
            </div>
            <div class="relationShipClick-box c-border-b" @click='handleClickTag3'>
              <choose-contacts title="直系亲属电话" v-model="formData.family" name="family" v-validate="'required'" />
            </div>
          </div>
          <div class="error-tip" v-if="errorInfo.family && fields.family && !fields.family.dirty">{{errorInfo.family}}</div>

          <choose-contacts title="同事电话" v-model="formData.colleague" name="colleague" v-validate="'required'" v-sina-ads="stat.publicInfo.contacts.colleague" />
          <div class="error-tip" v-if="errorInfo.colleague && fields.colleague && !fields.colleague.dirty">{{errorInfo.colleague}}</div>
          <choose-contacts title="朋友电话" v-model="formData.friend" name="friend" v-validate="'required'" v-sina-ads="stat.publicInfo.contacts.friend" />
          <div class="error-tip" v-if="errorInfo.friend && fields.friend && !fields.friend.dirty">{{errorInfo.friend}}</div>
        </div>

        <div class="c-button-group">
          <mt-button type="primary" size="large" class="btn-lg btn-primary btn-round" :disabled="!finished" @click.native="defendClick.wrap(submit)" v-sina-ads="stat.publicInfo.contacts.submit">提交</mt-button>
        </div>
      </div>
    </div>
