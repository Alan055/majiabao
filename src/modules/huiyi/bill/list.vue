        ...
        <mt-tab-container-item id="1" v-if="pageControl1.dataList.length" >
          <div class="c-view-content" @scroll="pageControl1.scroll($event)">
            ...
            >
              ...
              <!--列表数据-->
              <div class="bill-list">
                <bill-list-item v-for="item in pageControl1.dataList" :key="item.id" :item="item" v-sinaAds="{
        ...
        <mt-tab-container-item id="2" v-if="pageControl2.dataList.length" >
          <div class="c-view-content" @scroll="pageControl2.scroll($event) ">
            ...
            >
              ...
              <!--数据列表-->
              <div class="bill-list">
                <bill-list-item v-for="item in pageControl2.dataList" :key="item.id" :item="item" v-sinaAds="{
                  currEvent: adsInfo.bill.list.clickBillList,
                  currEventParams: {
                    type: 1
                  }
    ...
    methods: {
      gotoAgreements () {
        ...
        if (this.$root.isApp) {
          ...
          this.$root.openUrl({
            url: path + `/pages/my#/agreements/loanList/${this.routeParams.productid}/${this.routeParams.orderid}`,
            title: '借款协议'
          });
