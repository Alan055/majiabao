<template lang="html">
    <div class="home-page-detail view c-view">
        <div class="c-view-content">
            ...
            <div class="c-table-row c-justify-between el-bottom-content">
                <div class="el-edit">责任编辑：{{item.author}}</div>
                <div class="el-good">
                    <img :src="require(`../../assets/images/good-${Number(item.islike)? 'activate' : 'gay'}.png`)" @click="handleGoodClick(item)"/>
                    <span class="el-good-count">{{item.likenum || 0}}</span>
                </div>
            </div>
            <!-- 精彩推荐 -->
...
export default {
  ...
  methods: {
    ...
    handleGoodClick(item) {
      ...
      if (!item.islike) {
        item.islike = 1;
        item.likenum++;
        helper.set("panda-staging-news-detail-like", item.id);
        Toast("点赞成功");
      }
    }
  },
  created() {
    ...
    helper.get("panda-staging-news-detail-like").then(res => {
      if (this.item.id == res) {
        this.item.islike = 1;
      }
    });
