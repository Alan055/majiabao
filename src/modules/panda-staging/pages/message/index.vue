              ...
              <div class="message-list" ref="wrapper">
                <div class="message-item" v-for="item in list2"
                  ...
                  <div class="card">
                    ...
                    <div class="cardContent">
                      <span v-html="item.msgsummary"></span>
                    </div>
                  </div>
                </div>
                <!--暂无数据-->
...
<script>
	...
	export default {
		...
		methods: {
			...
			async fetchRedDot() {
				...
				await api.user.message.redDot({
						...
						if (!helper.isSuccess(res)) return;
						console.log("开始红点查询");
						// 第一个元素是我的消息  第二个元素是系统消息
						if (res.data.redDots[0]) {
							let first = res.data.redDots[0]
							if (first.positioncode == "2001004001") {
								this.redTypea = first.redtype;
								this.redIda = first.id;
								this.redDota = first.positioncode;
							} else if (first.positioncode == "2001004002") {
								this.redTypeb = first.redtype;
								this.redIdb = first.id;
								this.redDotb = first.positioncode;
							} else {
							}
						}
						if (res.data.redDots[1]) {
							let next = res.data.redDots[1]
							if (next.positioncode == "2001004001") {
								this.redTypea = next.redtype;
								this.redIda = next.id;
								this.redDota = next.positioncode;
							} else if (next.positioncode == "2001004002") {
								this.redTypeb = next.redtype;
								this.redIdb = next.id;
								this.redDotb = next.positioncode;
							} else {
							}
						}
						//最新一条消息类型tab默认
						await api.user.message
							.msgType({
								productId: this.productId,
								token: this.token
							})
      ...
      async init(cb){
        ...
        await this.fetchRedDot();
	      cb && cb() // 这个是页面框架的回调  一定要写
      }
		},
		watch: {
			deep: true,
