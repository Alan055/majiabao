...
export default {
  ...
  mounted() {
    ...
    //   this.$AppBridge.goNative({pkg: "login"});
    // }
      ...
      this.$AppBridge.getUserInfo({}, res => {
        if (res && res.data && res.data.token) {
          ...
          this.$AppBridge.goNative({
            pkg: "login",
            url: process.env.staticPath + "/panda-staging/#/bill"
          });
        }
      });
    } else {
  ...
  onRefresh(tag, data) {
    console.log("进入到onRefresh钩子里面来了", tag, data);
    if (tag == "login") {
...
.c-view-content {
  /deep/ .van-tabs__line {
    left: 36px; //这个left可能不准，需要在使用到的组件里再重置
  }
}
