        ...
        methods: {
            gotoAgreements() {
                ...
                if (this.$root.isApp) {
                    ...
                    let path = location.href.split('/panda-staging')[0];

                    //  pages/panda-staging#/bill
                   // let url = path + `/pages/my/#/agreements/loanList/${this.routeParams.productid}/${this.routeParams.orderid}`;

                    let url = path + `/panda-staging/#/agreements/loanList/${this.routeParams.productid}/${this.routeParams.orderid}`;
                 ...
                 /*   console.error('gotoAgreements===========>',url);
                    return*/
                    this.$root.openUrl({
                        url: url,
                        title: '借款协议'
                ...
                Promise.all([this.getBillList1(), this.getBillList2()]).then((values) => {
                    ...
                    billList.forEach((item) => {
                        if(item.dueamount){
                            ...
                            count = new Number(parseFloat(count) + parseFloat(item.dueamount)).toFixed(2);
                         //   this.loanMoney = (Number(this.loanMoney) + (Number(item.dueamount)).toFixed(2));
                        }
                    })
                    this.loanMoney =count
